...
关于这个例子，f3是我加上去的。作者认为，main方法中调用f2的时候，类型参数被捕获了，或许是吧，但这个例子其实无法证明。首先t.getClass()获取的是类的实际类型，所以打印永远是正确的，无论
有没有捕获。要证明是否捕获，其实要通过方法的返回类型来判断(这里的逻辑有点跳跃，但我会稍后解释)。方法f3佐证了我的观点。